...
Status: approved scoped refactor implemented; local hard-cutover gates and independent final review pass. Current implementation, verification, coverage limits and publication status are authoritative in `pi-fabric-arbor/acceptance-ledger.md`. PR13 hard-cutover acceptance now passes within the recorded scope; publication is verified separately.

Historical milestone notes below do not override the current ledger. The revised owner-local PR0 gate, repaired PR1 source-only package/retained-characterization gate, and reviewed PR2 bounded managed execution substrate PASS on local deterministic lanes recorded in `pi-fabric-arbor/acceptance-ledger.md`. PR2 accepts A02/A26 only within that bounded substrate; broader A11/A29 remain PARTIAL. A01 and previously accepted read-only/package portions of A17/A22/A30 are preserved. The PR3 transactional interface and final adversarial repairs PASS scoped normal/native gates, authorizing the PR3 milestone checkpoint. PR4 supplied committed-material evaluator implementation, all five supplied review repairs and additional exact-filename/null-preset repairs PASS independent scoped normal/native verification as recorded in `pi-fabric-arbor/docs/pr4-evaluator-evidence.md`. PR5 owned material and all six supplied repairs PASS independent scoped verification, including adjacent material control-state repairs, as recorded in `pi-fabric-arbor/docs/pr5-material-evidence.md`. The PR5 publication gate is accepted; this does not accept full journeys. PR6's two serial autonomous code/instruction journeys, five supplied fixes and adjacent exact-keep/provider-admission repairs PASS independent scoped normal/native/audit verification recorded in `pi-fabric-arbor/docs/pr6-research-evidence.md`. The user-authorized PR6 publication gate is scoped to that evidence; PR #3 stays draft. Separately authorized PR7 branching/parallel implementation passes its scoped normal/native/audit gate recorded in `pi-fabric-arbor/docs/pr7-parallel-evidence.md`; the four independently supplied repairs and adjacent admission/wave-error fixes pass the repaired gate and independent read-only source/parity review. PR7 publication was verified at pushed HEAD `f1c816c5e54218066b99cf45fafce80b18afad11`. Separately authorized PR8 controls, exact partial continuation, recovery and source apply/undo pass scoped gates in `pi-fabric-arbor/docs/pr8-recovery-evidence.md`: normal275, targeted23 and native42, with all independently reported reload, terminal-source, source-preflight and continuation findings repaired by executed regressions. Final independent guard and Main whole staged-diff reviews pass; user-authorized publication requires verified ordinary push. PR #3 stays draft. PR9 now passes independent Main review of the supplied diff and repaired scoped gates in `pi-fabric-arbor/docs/pr9-held-out-evidence.md`: normal292, targeted17, native13 plus full native PR6/PR7 21/5. Stale final admission and test-linked ideation/convergence leaks were reproduced and repaired. User-authorized publication requires full staged review and ordinary remote equality; PR #3 remains draft. PR10 now passes independent full read-only review and deterministic local grounding/experience gates: normal307, targeted15, native PR10 four source/installed cases, preserving native PR6/PR7/PR8/PR9 21/5/42/13. Exact scope and publication status are in `pi-fabric-arbor/docs/pr10-grounding-evidence.md`. PR11-PR13 and broader research/product acceptance remain outstanding.
...
Build a source-loaded Pi package whose **passive Fabric configuration parent `arbor` contains one operational managed `arbor.owner` and one non-operational sibling `arbor.drain` lifetime guard in the owning Pi host. The owner owns operational services and deterministic bounded dispatch/collection while a persistent Fabric actor chooses research direction by returning structured proposals**. Use short-lived Fabric agents for individual experiments. Keep a small transactional research store and Git/artifact workspace layer; do not implement a second agent runtime, persistent coordinator loop, mailbox, or participant registry.
...
Owning Pi commands/tools
          ...
          v
passive arbor configuration parent (does not await child readiness)
  +--> arbor.drain: non-operational scoped lifetime guard
  |      acquires arbor_lifetime.lease from the sibling owner
  +--> arbor.owner: sole operational owner in the existing Pi host
  captures original component context.call and signal; native work starts after activation
          |
...
Register definitions using public `pi-fabric/protocol` component registration/discovery events. Keep one declarative enabled application entry `{id: "arbor", component: "arbor"}`, preserving unrelated configuration, and one public `fabric-arbor` skill. The passive configuration parent declares one operational `arbor.owner` and one non-operational sibling `arbor.drain`. It does not await child readiness. Definition name, configured application ID, internal child IDs, provider namespaces and run actor IDs are separate identities.

**Approved scoped lifetime convention (A02/A22/A26):** owner activation opens owner-local services, captures the original `FabricComponentContext.call` and signal unchanged, stages providers through `context.provide`, and registers generation-scoped cleanup. The ten native `ARBOR_OWNER_REFS` remain unchanged, as do finite definition-time optional catalog requirements. No native call, experiment or actor bootstrap runs during owner activation. Native operations begin only after owner activation commits, through exact declared refs on captured `context.call(ref, args)`.

The owner provides the existing 21 `arbor.*` product/diagnostic refs plus discoverable, acquisition-only `arbor_lifetime.lease`. The lease has closed empty-object arguments (no additional properties), null output, risk `agent`, and effect `{kind: "scoped", ordering: "ordered", resources: ["arbor:owner:lifetime"]}`. It is a lifecycle capability, not a product operation, native dependency or mutation transport. The guard requires only `arbor_lifetime.lease` and acquires exactly that resource during its activation. This scoped acquisition is the sole activation-time capability exception. The guard has no native effects, actor bootstrap, operational services, participant registry or transport. Its lease inverse awaits that generation's `ResearchService.dispose()`, not storage close.

All new mutation admission requires read-only, generation-bound guard status and ownership of that generation's lease. A published provider or active parent alone cannot admit work. Missing acquisition, partial activation, guard retirement and generation mismatch fail closed. Keep service-level monotone draining and post-await admission checks, including rejection before any new effect after an awaited boundary. Internal cleanup remains independent of the new-mutation gate so late handle settlement, stops and evidence persistence can finish after admission closes. Partial activation must still dispose any created owner resources through registered generation-owned cleanup. Cleanup failures remain explicit and cannot become successful activation, settlement or permission to close supporting storage early. This approval establishes architecture, not production acceptance or a runtime inventory.
...
| Capability lifetime | Sole operational owner, original captured context/signal, exact `requires`, and non-operational sibling's scoped owner-lifetime lease | Global prepared-provider pointer, readiness certificates, or second operational owner |
...
- The actor requires only `agents.self`, never Arbor mutation refs or `arbor_lifetime.lease`. The owner supplies authoritative observations and any selected grounded sources, and validates every proposal.
...
Required teardown order is **admission retirement -> scoped drain with real settlement -> native owner unload/abort -> eventual `arbor` provider `close()` for supporting storage**. Retirement of new mutation admission is distinct from native owner context unload/abort. Guard withdrawal closes admission, and its scoped lease inverse awaits the generation's `ResearchService.dispose()` while the original owner call seam and signal remain usable for settlement. Mark the service monotonically draining, settle in-flight launch results, stop tracked owned work through public APIs, and record interruption/cleanup status independently of the mutation gate. An abort-raced public wrapper is not real native settlement.

Fabric controls live provider binding retirement, dependent withdrawal and inverses. This is not a retire-last Fabric directive. Already committed views may retain the retiring generation. Keep storage and supporting resources valid until eventual `arbor` provider `close()` after applicable retainers/in-flight calls release and disposal settles. The lease inverse does not close storage. Register generation-owned cleanup during activation, including partial activation, and prove idempotence without double-close. Cleanup failure remains explicit, with unresolved work and evidence retained, not a claim of clean unload. Read-only clients own no execution resources. The settlement path in section 4.4 must be proven independently of actor outbox delivery or later actor activation.
...
| `src/component/definitions.ts` | Passive application parent, sole operational owner publication and non-operational sibling lifetime guard; original captured `context.call`/signal, exact native dependencies, scoped lease acquisition during guard activation only, bounded dispatch/settlement and retained-storage lifetime; no native effects or actor bootstrap during activation |
...
**Observed repaired gate:** exact PR6 base `e8abded64db245ebfca2725bdeca486094e0c1a4`, preserving supplied PR7 diff. Four independently supplied preparation/restore/review/lineage findings and adjacent prelaunch admission/wave-error masking bugs repaired. Seven source probes red then green; targeted **67**, final normal **252**, native PR2/3/4/5/6 **8/20/21/9/21**, final PR7 native **5** (A12, pause/cancel, preparation/protected committed-worker effects). Independent read-only repair/parity review found no blockers. Whole staged review additionally found retained-PR6 projection compatibility; only additive output fields are now optional, preserving saved identities, with a red/green retained-row regression and full schema-impacted native PR3/PR6 **20/21** reruns. A subsequent whole review found shared-ancestor insight overflow; the unchanged100 bound is now enforced transactionally, with sibling red/green regression and another native PR6 **21/21**. Strict audit **163 two-window exits** (160 clean plus three expected synthetic PR2 failures), including eight earlier PR7 checkpoint hosts without relabeling red assertions. Identical two-candidate atomic waves, one discarded warmup then three fixed one-second waves per mode: concurrency-one median **3524 ms**, concurrency-two **1962 ms**, ratio **0.5567536889897844**, overlap **988/960/984 ms**. Environment/overhead retained; no intervening actor ask. Reviewer-verified historical 1959/3497 and prior split-dispatch results remain historical. See `pi-fabric-arbor/docs/pr7-parallel-evidence.md`. User-authorized publication is gated by full scoped staged-diff review and remote equality; PR #3 stays draft, PR8-13 outstanding. Controlled wave metric only, not campaign speed or research-quality evidence.
...
**PR8 scoped gates PASS:** continued at verified PR7 `f1c816c5e54218066b99cf45fafce80b18afad11`, preserving all33 supplied staged paths and adding only continuation-aware `src/research/tree.ts`. Actual-native pending Direction/Review/Collaborative and material review reloads renew an unapproved binding before fresh UI. Original immutable source intents adopt only complete exact postimages after journal/receipt gaps under fresh source approval and normal Fabric write policy, then exact undo succeeds even if resumed research advances the incumbent. Terminal source reconciliation after cancel/reload retains the cancelled state and original research generation, admits only an original same-owner intent and launches no actor/worker. Newer bytes/modes, another native owner and write deny/ask still block. Further full-diff review repairs cover dangling-parent preflight, measured continuation refinement/latest-attempt discard and cancel-during-collection terminal monotonicity, with real-FS/service red/green plus native continuation journeys. Final normal **275**, targeted **23**, full native PR8 **42**, no skips and naturally complete totals; both no-emit checks and exact audit pass. Native PR3/5/6/7 rerun **20/9/21/5**; PR2/4 **8/21** remain retained prior repair gates. Incomplete exit0 and failed output-overflow logs are not accepted totals. All nine killed/reopened boundaries prove owner-loss **block**, not portable resume. A12 latest ratio **0.565031374786081** preserves the <=0.8 controlled-wave target. Evidence/ledger record exact roots,711 classified exits and A09/A10 plus PR8 portions of A11/A24/A27/A28/A29. Final independent guard and Main whole staged-diff reviews pass, with publication targeted23/23 and audit PASS; ordinary push equality remains required. PR #3 stays draft. No full product or PR9-PR13 acceptance.
...
**Independently reviewed PR9 gate:** verified exact worktree/branch/start HEAD `7a03483910d0d98e2ac1ee7feccbb295cf002482`, preserving all supplied24 modified/five untracked paths. Separate frozen development/held-out/final records gate integration transactions and new source apply. Explicit owner-only execute-policy `/arbor validate RUN ATTEMPT` selects one exact final pair, never an actor threshold loop; snapshot/revision/capacity prerequisites precede consumption. Main review reproduced and repaired stale held-out snapshots consuming final evidence, test-linked interpretations leaking into ordinary actor observations, and test-only decisions driving development convergence. Actual source/clean-installed native **13/13**, normal **292/292**, targeted **17/17** and exact public/package/read-only audit PASS. Same-OID/stale-review/final-reuse, minmax, alias and bounded-policy adversarial probes pass. Full native PR6/PR7 reruns **21/5** preserve both five-stage journeys and unchanged A12 (3493ms serial,1976ms parallel, ratio0.5657028342399084; all three warmed waves overlap). Prior native PR3/4/5/8 **20/21/9/42** and original normal/target/native **288/13/12** remain audited historical evidence, not new reruns. A14 and the PR9 A05/A06/A08/A20/A21 portions pass this bounded independent review; no sealed-data or broader generalization claim. User-authorized publication requires full staged review and ordinary remote equality; PR #3 stays draft and PR10-PR13 remain outstanding. Exact evidence and phase checkpoints: `pi-fabric-arbor/docs/pr9-held-out-evidence.md`.
...
**Independently reviewed deterministic PR10 gate:** finite optional definition-time search/fetch, owner-visited artifacts, exact native literature/source provenance, checked grounded hypotheses, fresh-v2 project lesson retrieval and actual proposal/context/action/outcome exports pass scoped A15 and PR10-literature A25 gates. Earlier experience findings and a full-review direction-lesson descendant-source omission were reproduced/repaired. Successful independent full read-only review reports no blocking issue. Normal307, targeted15, native PR10 four source/installed cases and preserved full native PR6/7/8/9 21/5/42/13 pass; isolated unchanged A12 ratio is0.558865. Exact strengthened reruns, review limitations and publication status are in `pi-fabric-arbor/docs/pr10-grounding-evidence.md`. User-authorized publication still requires reviewed explicit-path staging and normal remote equality. PR11-PR13 remain outstanding; PR #3 stays draft.
...
**PR12 recovery checkpoint:** implementation uses `SourceView` over existing read-only ResearchStore transactions for Pi/CLI/browser, recorded-parent candidate diffs, owning-Pi no-ID intake/selection through normal Fabric policy, native Fabric navigation and owner-only bounded exports. Cold readers now use disposable read-only EXCLUSIVE admission, with original-error preservation, instead of relying on header inspection alone. Source and clean-installed strict storage checks, normal checks and both actual Pi/CLI/browser journeys pass. The exact counts, commands, environment and retained historical failures are in [PR12 presentation evidence](../../pi-fabric-arbor/docs/pr12-presentation-evidence.md). The pre-guard held-spawn reload path lost authoritative handle delivery on the declared runtime and in a disposable published-host exploration. The approved scoped lifetime guard addresses that boundary. Production scoped lifetime gates now verify source/clean-installed whole-Pi held ask/spawn, root application held research spawn and all-five-ref substrate settlement. The evidence and ledger own exact results, independent review and remaining milestones. Together with the final full regression/audit and independent reconciliation review, these proofs satisfy the required PR12 evidence within the explicitly approved scoped limits; they do not establish universal replacement behavior. Public internal-owner reload reports `Unknown Fabric component: arbor.owner`, so neither owner-only reload nor dependency-driven replacement is proven. Target the root application for component reload. Catalog changes still require quiescent maintenance and re-registration. No Fabric-internal change, dependency upgrade or relaxed assertion was made. The full milestone/native reconciliation and both audits now pass, and the spec supervisor accepts PR12 publication within the recorded limits. PR13 is dependency-ready after verified publication; no retained user artifact deletion is authorized.
...
**Executed local checkpoint:** PR12 publication was verified at `b509ae7ccb1db2da02272d9eb9d304267c59119f`. PR13 removes only 284 tracked package-owned legacy files and preserves all 43 accepted production modules and declared dependencies byte-for-byte. Useful arithmetic/source/recovery contracts run against current interfaces, not the retired v1 test runtime. Source308, targeted9, every post-cutover PR2–PR12 native lane (187 cases, including isolated unchanged A12) and final fresh-package9 gates pass, as does the mechanical A18/A19/A22/A30 audit. Historical milestone records remain repository-only, while shipped instructions contain current commands. Independent review/publication and exact limits are authoritative in the acceptance ledger. Existing runtime/user artifacts remain untouched.
...
| A02 | One passive `arbor` configuration parent has one operational `arbor.owner` and one non-operational sibling `arbor.drain`; the parent does not await child readiness. The owner invokes the persistent proposal actor through the original captured post-activation `context.call` and signal with the unchanged ten native `ARBOR_OWNER_REFS`. Only the guard acquires `arbor_lifetime.lease` during activation, with no native effects, actor bootstrap, services, registry or transport. No custom reasoning loop or private imports. |
...
| A22 | Four product facade and six owner research operations are implemented; every registered provider ref, command mapping, schema, caller class, risk and effect is listed in the PR3 manifest, including any narrower review/apply/undo routes. The existing 21 `arbor.*` product/diagnostic refs remain unchanged. Add only discoverable acquisition-only `arbor_lifetime.lease`, with closed empty args/null output, risk `agent` and scoped/ordered effect resource `arbor:owner:lifetime`, to the manifest. Actor proposal schemas are closed and its commitment is only `agents.self`, without lifetime or Arbor mutation refs. Exactly one public `fabric-arbor` skill is discovered; coordinator/executor/literature documents and conditional references are packaged but not registered as skills. All eleven upstream skills have an explicit merge/delete/role disposition. One configured application entry matches the passive parent and its owner/guard derived definitions. |
...
| A26 | PR0/PR2: obtain and immutably record the native owning Pi root/host/owner identity through public `agents.self`, plus component instance/generation, material/cwd/OID/policy, actor, and worker identities; prove the operational owner preserves the original `context.call` and signal, returns from activation before native calls, and uses the unchanged ten native `ARBOR_OWNER_REFS` plus only finite declared optional catalog refs. Separately prove the non-operational guard requires only `arbor_lifetime.lease` and acquires exactly `arbor:owner:lifetime` during activation, with no parent readiness wait. All new mutation admission requires read-only generation-bound guard status and lease ownership, preserves service monotone draining/post-await checks, and cannot gate internal cleanup. A structured `agents.ask` proposal settles before owner-launched workers continue; owned waits collect them and a bounded later ask receives fresh observations with no Main inference. CLI has no mutation attachment. |
...
| A29 | PR0/PR2/PR8 and scoped lifetime revision: admission retirement -> scoped drain with real generation `ResearchService.dispose()` settlement -> native owner unload/abort -> eventual `arbor` provider `close()` for supporting storage. No retire-last Fabric directive. The lease inverse never closes storage, and retained old-generation calls keep valid support until provider close. Partial activation and cleanup failure remain explicit, cleanup is idempotent and generation-owned, and no stale mutation or double-close is admitted. Owner-only/dependency-driven replacement requires separate proof, not inference from root reload evidence. |
