...
export const lessonProvenanceSchema = closed({runId:id,revision:integer(),materialId:id,epoch:id,specId:str(64),sourceIds:array(id,128),uninspectedSourceRefs:array(str(4096),128),materials:array(str(96),64),applicability:str(8192),outcome:str(64)});
...
const artifactSchema: Schema = { oneOf: [sourceAccessSchema(), sourceInspectionSchema(), closed({ id, kind: enumeration("native-evidence"), attemptId: id, generation: str(), nativeId: str(), materialId: id, epoch: id, status: enumeration("completed", "failed", "stopped", "timed_out"), digest: str(64), summary: { type: "string", maxLength: 1024 }, validation: enumeration("unscored-native-observation") }), closed({ id, commandId: id, path: str(4096), digest: str(64), kind: enumeration("unscored-json-export", "source-operation") })] };
