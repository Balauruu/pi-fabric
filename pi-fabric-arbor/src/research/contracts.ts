...
export const CONFIG_SCHEMA = partial({
  ...
  roleTools: partial({ coordinator: array(enumeration("fabric_exec"), 1, 1), executor: array(enumeration("read", "grep", "find", "ls"), 4) }),
  ...
  sourceRefs: array(str(4096)), preset: nullable(str()), execution: enumeration("inspect", "deferred", "evaluate"),
});
...
export const RESEARCH_ACTIONS: FabricActionDescriptor[] = [
  action("start", START_SCHEMA, "agent", "emission", "Owning Pi: freeze a run; inspect native observations or evaluate an explicit exact-material pair. No candidate editing, incumbent adoption or source apply."),
