...
export const CONFIG_SCHEMA = partial({
  ...
  search: partial({ maxDepth: integer(10, 1), maxChildren: integer(10, 1), concurrency: integer(2, 1), maxActorTurns: integer(24, 1), mode: enumeration("auto", "direction", "review", "collaborative") }),
  ...
  sourceRefs: array(str(4096)), preset: nullable(str()), execution: enumeration("inspect", "deferred"),
});
...
export const evaluatePayload = closed({ attemptId: id, evaluationId: id });
...
export const ACTOR_PROPOSAL_SCHEMA: Schema = { oneOf: Object.entries(PAYLOADS).map(([kind, payload]) => closed({ ...BINDING, version: { type: "integer", enum: [2], minimum: 2, maximum: 2 }, kind: enumeration(kind), expectedEvidence: array(id), estimatedBudget: closed({ attempts: integer(2), evaluatorCalls: integer(100) }), rationale: str(4096), payload })) };
...
export const RESOLVED_SPEC_SCHEMA = closed({ version: { type: "integer", enum: [2], minimum: 2, maximum: 2 }, config: complete(CONFIG_SCHEMA), origins: closed(originFields(CONFIG_SCHEMA)), identity: str(64), source: closed({ root: str(4096), oid: nullable(str(64)), materialId: id, capture: enumeration("source-reference-not-candidate-snapshot") }), roles: closed({ coordinator: roleSchema, executor: roleSchema, subject: roleSchema }), enforcement: closed({ attempts: enumeration("transactional"), evaluatorCalls: enumeration("unavailable-PR4"), activeTime: enumeration("dispatch-admission"), tokens: enumeration("observational"), cost: enumeration("observational"), artifacts: enumeration("export-admission") }) });
export const RUN_SCHEMA = closed({ id, spec: RESOLVED_SPEC_SCHEMA, requestHash: str(64), owner: nativeOwnerSchema, componentId: str(), generation: str(), epoch: id, revision: integer(), state: enumeration("ready", "running", "paused", "awaiting_review", "completed", "cancelled", "interrupted", "cleanup_pending", "failed"), attemptsUsed: integer(100), active: integer(2), createdAt: integer(Number.MAX_SAFE_INTEGER), activeMs: integer(Number.MAX_SAFE_INTEGER), activeSince: nullable(integer(Number.MAX_SAFE_INTEGER)), steering: array(str(4096), 16), pendingDecisionId: nullable(id), execution: str(1024), error: nullable(str(4096)) });
...
const action = (name: string, inputSchema: Schema, risk: FabricRisk, kind: "none" | "transactional" | "emission", description: string): FabricActionDescriptor => ({ name, inputSchema, outputSchema: name === "start" || name === "inspect" ? nullable(PROJECTION_SCHEMA) : RECEIPT_SCHEMA, risk, effect: effect(kind, name === "apply" || name === "undoApply" ? "arbor:source" : "arbor:research"), description });
export const RESEARCH_ACTIONS: FabricActionDescriptor[] = [
  action("start", START_SCHEMA, "agent", "emission", "Owning Pi: resolve/freeze a fresh run and optionally run bounded READ-ONLY native observations. No scoring or candidate edits before PR4+."),
  ...
  action("export", EXPORT_SCHEMA, "write", "emission", "Owning Pi: generate an unscored JSON research projection artifact idempotently. Never writes source material."),
  ...Object.entries(ACTION_SCHEMAS).map(([name, schema]) => action(name, schema, name === "dispatch" ? "agent" : name === "evaluate" ? "execute" : "write", name === "dispatch" || name === "evaluate" ? "emission" : "transactional", `Owner-only ${name}: exact bindings, reservations and evidence. Actor proposals are not authorization.${name === "evaluate" ? " Evaluators unavailable until PR4; caller scores forbidden." : ""}`)),
  action("review", REVIEW_SCHEMA, "write", "transactional", "Owning Pi UI only: request actual user response to exact pending decision after host permission. No supplied approval or receipt accepted; no measured keep/apply."),
...
export const COMMAND_ACTIONS = Object.freeze({ start: "start", show: "inspect", pause: "control", resume: "control", cancel: "control", steer: "control", keep: "decide", discard: "decide", review: "review", export: "export", apply: "apply", "undo-apply": "undoApply" } as const);
export const ACTION_MANIFEST = RESEARCH_ACTIONS.map(descriptor => ({ ref: `arbor.${descriptor.name}`, caller: descriptor.name === "inspect" ? "reader" : descriptor.name === "review" ? "owning-Pi-user-dialog" : "owning-Pi", commands: Object.entries(COMMAND_ACTIONS).filter(([, action]) => action === descriptor.name).map(([command]) => `/arbor ${command}`), actorCommitment: false, ...descriptor }));
