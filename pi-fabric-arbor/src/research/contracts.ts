...
const attemptSchema = closed({ slotReserved: {type:"boolean"}, selection: selectionSchema, waveId: id, parentIncumbent: str(64), evaluationReservation: integer(10000), id, nodeId: id, task: str(4096), state: enumeration("reserved", "running", "completed", "failed", "stopped", "timed_out"), nativeId: nullable(str()), nativeDigest: nullable(str(64)), evidenceId: nullable(id), model, materialId: id, epoch: id, generation: str() });
attemptSchema.required = attemptSchema.required!.filter(k => !["slotReserved", "selection", "waveId", "parentIncumbent", "evaluationReservation"].includes(k));
