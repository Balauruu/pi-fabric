...
export interface Config {
  ...
  sourceRefs: string[]; preset: string | null; execution: "inspect" | "deferred";
}
export interface ResolvedSpec {
  version: 2; config: Config; origins: Record<string, string>; identity: string;
  ...
  enforcement: { attempts: "transactional"; evaluatorCalls: "unavailable-PR4"; activeTime: "dispatch-admission"; tokens: "observational"; cost: "observational"; artifacts: "export-admission" };
}
...
export async function resolveSpec(cwd: string, profile: Record<string, unknown>, project: Record<string, unknown>, overrides: Record<string, unknown>, activeModel?: string): Promise<ResolvedSpec> {
  ...
  for (const [origin, layer] of [["built-in", defaults(cwd)], ["profile", profile], ["project", project], ["explicit", overrides]] as const) { validate(CONFIG_SCHEMA, layer); merge(resolved, layer, origin); }
  ...
  const roles: ResolvedSpec["roles"] = {
    coordinator: { model: config.roles.coordinator, origin: origins["roles.coordinator"]!, instructionsId: digest(COORDINATOR_INSTRUCTIONS), tools: ["fabric_exec"], requires: ["agents.self"], resultContract: "arbor.actor-proposal.v2" },
    executor: { model: config.roles.executor, origin: origins["roles.executor"]!, instructionsId: digest(EXECUTOR_INSTRUCTIONS), tools: ["read", "grep", "find", "ls"], requires: [], resultContract: "native-terminal-unscored-text" },
  ...
  const body = { version: 2 as const, config, origins, source, roles, enforcement: { attempts: "transactional" as const, evaluatorCalls: "unavailable-PR4" as const, activeTime: "dispatch-admission" as const, tokens: "observational" as const, cost: "observational" as const, artifacts: "export-admission" as const } };
