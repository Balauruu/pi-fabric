...
export async function resolveSpec(cwd: string, profile: Record<string, unknown>, project: Record<string, unknown>, overrides: Record<string, unknown>, activeModel?: string): Promise<ResolvedSpec> {
  ...
  if (!["material", "research"].includes(config.execution) && config.roleTools.executor.some(t => !["read", "grep", "find", "ls"].includes(t))) throw new Error("Writable executor tools require explicit material execution; inspect stays read-only");
  ...
  if (["evaluate", "material", "research"].includes(config.execution)) {
    ...
    if (config.objective.qualityVetoes.some(v => !["no-native-failures", "all-tasks-correct", "preserve-baseline-correct"].includes(v)) || (evaluation.kind !== "agent-suite" && config.objective.qualityVetoes.some(v => ["all-tasks-correct", "preserve-baseline-correct"].includes(v)))) throw new Error("Unsupported selected quality veto for evaluator");
