...
export function sourceInspectionSchema(){return closed({id,digest:str(64),kind:enumeration('source-inspection'),...provenance(),accessId:id,url:str(4096),title:str(512),artifact:artifact(),passage:str(4096),claim:str(4096),limitations:str(4096),nativeId:str(),requestId:str(64),roleBundleId:str(96),model:str(),validation:enumeration('source-linked-hypothesis-not-grade')});}
export function literatureResultSchema(){return closed({sentinel:enumeration('ARBOR_LITERATURE_RESULT_V1'),batchId:id,sources:array(closed({accessId:id,passage:str(4096),claim:str(4096),limitations:str(4096)}),4),blocked:nullable(str(4096))});}
