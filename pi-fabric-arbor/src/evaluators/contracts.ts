...
export interface MaterialRef { root: string; oid: string; files: string[]; format?: "git-tree" }
export interface Snapshot extends MaterialRef { id: string; directory: string; contents: Record<string, string>; executable: Record<string, boolean>; entries?: Record<string, { mode: string; base64: string }> }
...
export function definitionSchema(): Schema {
  const material = closed({ root: str(4096), oid: { ...str(64), pattern: "^(?:capture|[a-f0-9]{40}|[a-f0-9]{64})$" }, files: array(str(4096), 4096, 1), format: enumeration("git-tree") }, ["root", "oid", "files"]);
...
export interface CommandCheck { id:string; requestId:string; state:"reserved"|"launching"|"native-complete"; nativeId:string|null }
...
export interface EvaluationRecord {
  ...
  split?: import("./validation.js").Split; developmentId?: string | null;
  ...
  validity: "pending" | "valid" | "invalid"; incumbentDecision: "not-evaluated-PR5" | "separate-owned-decision"; error: string | null;
}
export const evaluationSummary = (e: EvaluationRecord) => ({ split: e.split ?? "development", developmentId: e.developmentId ?? null, attemptId: e.attemptId ?? null, epoch: e.epoch, specId: e.specId, baselineOid: e.snapshots.baseline.oid, candidateOid: e.snapshots.candidate.oid, id: e.id, purpose: e.purpose, state: e.state, definitionId: e.definitionId, baselineSnapshot: e.snapshots.baseline.id, candidateSnapshot: e.snapshots.candidate.id,
  invocationCount: evaluationCalls(e), invocations: e.invocations.map(i => ({ checkInvocationIds: (i.commandChecks??[]).map(c=>c.id), checkNativeIds: (i.commandChecks??[]).map(c=>c.nativeId), id: i.id, taskId: i.taskId, condition: i.condition, purpose: i.purpose, role: i.role, model: i.model, tools: i.tools, bootstrapId: i.bootstrapId, parentId: i.parentId, snapshotId: i.snapshotId, nativeId: i.nativeId, nativeStatus: i.native?.status ?? null, exitCode: i.native?.exitCode ?? null, error: i.native?.error ?? i.reason, deadline: i.native?.deadline ?? null, usage: i.native?.usage ? JSON.stringify(i.native.usage) : null, elapsedMs: i.native?.elapsedMs ?? null, valid: i.valid, score: i.score })), validity: e.validity, quality: e.quality, bindingGenerations: e.bindings.map(b => b.generation), analysis: JSON.stringify(e.analysis), incumbentDecision: e.incumbentDecision, error: e.error });
export function summarySchema(): Schema { return closed({ split: enumeration("development", "held-out", "final"), developmentId: nullable(id), attemptId: nullable(id), epoch: id, specId: str(64), baselineOid: str(64), candidateOid: str(64), id, purpose: enumeration("candidate", "feedback", "recheck"), state: enumeration("running", "INTERRUPTED", "completed", "blocked"), definitionId: str(64), baselineSnapshot: str(), candidateSnapshot: str(), invocationCount: integer(10000), invocations: array(closed({ checkInvocationIds:array(id,16), checkNativeIds:array(nullable(str()),16), id, taskId: id, condition: enumeration("baseline", "candidate"), purpose: enumeration("baseline", "candidate", "feedback", "retry", "recheck", "judge"), role: enumeration("subject", "judge", "command", "provider"), model: nullable(str()), tools: array(str(), 4), bootstrapId: str(64), parentId: nullable(id), snapshotId: str(), nativeId: nullable(str()), nativeStatus: nullable(enumeration("completed", "failed", "stopped", "timed_out")), exitCode: nullable(integer(255)), error: nullable(str(4096)), deadline: nullable({ type: "boolean" }), usage: nullable(str(1024)), elapsedMs: nullable(integer(Number.MAX_SAFE_INTEGER)), valid: { type: "boolean" }, score: nullable(str(80)) }), 10000), validity: enumeration("pending", "valid", "invalid"), quality: closed({ required: array(str(), 32), passed: { type: "boolean" }, limitedValidation: { type: "boolean" } }), bindingGenerations: array(str(), 128), analysis: str(32768), incumbentDecision: enumeration("not-evaluated-PR5", "separate-owned-decision"), error: nullable(str(4096)) }); }
export function evaluationCalls(e:{invocations:Array<{commandChecks?:unknown[]}>}):number {return e.invocations.reduce((n,i)=>n+1+(i.commandChecks?.length??0),0);}
export function nativeSuccess(n: NativeEvidence, requiredChecks?: number): boolean { return (requiredChecks === undefined || (n.checks.length === requiredChecks && (n.checkResults?.length ?? 0) === requiredChecks && (n.checkResults??[]).every(c=>nativeSuccess(c)))) && n.status === "completed" && n.error === null && !n.deadline && (n.exitCode === null || n.exitCode === 0) && n.checks.every(Boolean); }
