...
test("production definition is passive and declares exact requirements; provider close owns storage", async () => {
  ...
  assert.deepEqual((await provider.list({}, context())).map(d => d.name), ["runResearch", "resumeAttempt", "reviseRoles", "start", "inspect", "control", "export", "propose", "dispatch", "collect", "evaluate", "distill", "decide", "review", "apply", "undoApply", "substrateStart", "substrateInspect", "substrateCancel"]);
