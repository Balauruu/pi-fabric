...
test("PR5 native dirty baseline interruption reloads owner and preserves exact terminal baseline without redispatch", { timeout: 240000 }, async t => {
  const h = await host(`const starting=tools.call({ref:'arbor.start',args:{runId:'material'}});await Promise.race([tools.call({ref:'pr2fixture.ready',args:{}}),starting.then(()=>{throw new Error('Missing evaluation wait barrier')})]);const reload=components.reload({id:'arbor'});await tools.call({ref:'pr2fixture.release',args:{}});const interrupted=await starting;await reload;const before=await get();const r=before.run;const resumeArgs={runId:r.id,materialId:r.spec.source.materialId,epoch:r.epoch,revision:r.revision,commandId:'resume',action:'resume'};const resumed=await tools.call({ref:'arbor.control',args:resumeArgs});const duplicateResumed=await tools.call({ref:'arbor.control',args:resumeArgs});const recovered=await get();if(!recovered.run.material.baselineEvaluation)await act('evaluate',{attemptId:'baseline',evaluationId:'fresh-baseline',purpose:'recheck'},'fresh-baseline');${candidate}${keep}return JSON.stringify({interrupted,resumed,duplicateResumed,kept,p:await get()});`, { hold: "agents.wait" }); t.after(() => h.store.close());
  ...
  assert.deepEqual(h.value.resumed,h.value.duplicateResumed);assert.deepEqual(h.value.p.run.spec,h.value.interrupted.run.spec);assert.equal(h.value.p.evaluations.length,2,'Only original baseline plus candidate evaluation');
  const initial = h.store.evaluation("material", "evaluation-initial")!;
  assert.equal(initial.snapshots.baseline.oid,h.value.interrupted.evaluations[0].baselineOid);assert.equal(initial.snapshots.candidate.oid,h.value.interrupted.evaluations[0].candidateOid);
  assert.equal(new Set(initial.invocations.map(i=>i.nativeId)).size,2);for(const invocation of initial.invocations)assert.ok(h.events.some(e=>e.event==='native.result'&&e.data.ref==='agents.wait'&&e.data.result.id===invocation.nativeId&&e.data.result.status==='completed'),'Exact recovered native wait must match saved handle');
  assert.equal(initial.invocations.length, 2); assert.equal(initial.bindings.length, 2); assert.equal(initial.validity, "valid", h.root); assert.ok(initial.invocations.every(i=>i.nativeId&&i.native?.status==='completed'&&i.state==='ingested'),'Every recovered invocation needs exact terminal native evidence'); assert.equal(h.value.p.run.material.baselineEvaluation, "evaluation-initial"); assert.equal(initial.snapshots.baseline.oid,initial.snapshots.candidate.oid); assert.equal(h.events.filter(e => e.event === "material.subject").length, 4, 'Do not rerun a baseline whose native completion was recovered');
});
