...
// Provider evaluation must traverse the same agent/execute command composition
// as the product. Direct agent-risk start intentionally does not execute it.
const startViaPolicy = `const startViaPolicy=async(startArgs)=>{${commandProgram(researchCommand('start','{"runId":"policy-placeholder"}')).replace('{"runId":"policy-placeholder"}','startArgs')}};`;
...
test("PR4 actual external provider definition-time binding, effective schema and result rejection matrix", { timeout: 180000 }, async t => {
  const h = await host(`${catalogBind} ${startViaPolicy} const outputs=[];for(const mode of ['valid','invalid','failed','check']){await tools.call({ref:'pr4fixture.replace',args:{mode}});await components.reload({id:'arbor'});const p=await startViaPolicy({runId:'external-'+mode});outputs.push({mode,e:p.evaluations[0]});}await tools.call({ref:'pr4fixture.replace',args:{mode:'schema'}});await components.reload({id:'arbor'});let mismatch;try{await startViaPolicy({runId:'schema-mismatch'})}catch(e){mismatch=String(e)}return JSON.stringify({outputs,mismatch});`, { external: true, catalog: true, kind: "provider" }); t.after(() => h.store.close());
...
test("PR4 two-run optional provider replacement interrupts the shared owner; built-ins reactivate, unknown external completion blocks", { timeout: 180000 }, async t => {
  const h = await host(`${bind} ${catalogBind} ${startViaPolicy} const paths=await tools.call({ref:'pr4fixture.paths',args:{}});await tools.call({ref:'pr4fixture.gate',args:{}});const external=(async()=>{const p=await tools.call({ref:'arbor.start',args:{runId:'external-active'}});const r=p.run;return await tools.call({ref:'arbor.evaluate',args:{runId:r.id,materialId:r.spec.source.materialId,epoch:r.epoch,revision:r.revision,commandId:'initial-command-evaluation',payload:{attemptId:'exact-material',evaluationId:'evaluation-initial'}}});})();const builtin=tools.call({ref:'arbor.start',args:{runId:'builtin-active',overrides:{evaluator:{kind:'agent-suite',definition:paths.suite}}}});await Promise.all([tools.call({ref:'pr4fixture.ready',args:{}}),tools.call({ref:'pr2fixture.ready',args:{}})]);await tools.call({ref:'pr4fixture.replace',args:{mode:'valid'}});const reload=components.reload({id:'arbor'});await tools.call({ref:'pr2fixture.release',args:{}});await tools.call({ref:'pr4fixture.release',args:{}});await Promise.all([external,builtin]);await reload;const a=await tools.call({ref:'arbor.inspect',args:{runId:'external-active'}}),b=await tools.call({ref:'arbor.inspect',args:{runId:'builtin-active'}});const fresh=await tools.call({ref:'arbor.start',args:{runId:'builtin-after',overrides:{evaluator:{kind:'agent-suite',definition:paths.suite}}}});const p=await tools.call({ref:'arbor.inspect',args:{runId:'external-active'}});let blocked;try{await (async()=>{${commandProgram(researchCommand('resume','external-active'))}})()}catch(e){blocked=String(e)}return JSON.stringify({a,b,fresh,blocked});`, { external: true, catalog: true, kind: "provider", hold: "agents.wait" }); t.after(() => h.store.close());
...
test("PR4 review actual external provider request poisoning rejects all frozen bindings", { timeout: 180000 }, async t => {
  ...
  const h = await host(`${catalogBind}${startViaPolicy}const outputs=[];for(const field of ${JSON.stringify(fields)}){await tools.call({ref:'pr4fixture.replace',args:{mode:'poison-'+field}});await components.reload({id:'arbor'});const p=await startViaPolicy({runId:'poison-'+field});outputs.push({field,e:p.evaluations[0]})}return JSON.stringify(outputs);`, { external: true, catalog: true, kind: "provider" }); t.after(() => h.store.close());
