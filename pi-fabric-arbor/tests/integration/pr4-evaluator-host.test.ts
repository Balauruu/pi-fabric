...
test("PR4 exact effective public schemas, owner requirements and normal policy path", { timeout: 180000 }, async t => {
  const expected = ARBOR_ACTIONS.map(a => ({ name: a.name, inputSchema: a.inputSchema, outputSchema: a.outputSchema ?? null, risk: a.risk, effect: a.effect }));
  const h = await host(`const checks=[];for(const e of ${JSON.stringify(expected)}){const a=await tools.describe({ref:'arbor.'+e.name});checks.push(JSON.stringify(a.inputSchema)===JSON.stringify(e.inputSchema)&&JSON.stringify(a.outputSchema??null)===JSON.stringify(e.outputSchema)&&a.risk===e.risk&&JSON.stringify(a.effect)===JSON.stringify(e.effect));}const native=[];for(const ref of ${JSON.stringify(ARBOR_OWNER_REFS)}){const a=await tools.describe({ref});native.push({ref,closed:a.inputSchema.additionalProperties===false});}return JSON.stringify({checks,native});`); t.after(() => h.store.close());
