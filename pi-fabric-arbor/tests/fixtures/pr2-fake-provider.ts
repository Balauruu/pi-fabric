    ...
    streamSimple(model, context, options) {
      ...
      if (context.systemPrompt?.includes("proposal-only coordinator")) {
        ...
        if (data.research) {
          ...
          else if(pending){p.kind='dispatch';p.payload={nodeId:pending.nodeId,attemptId:pending.nodeId,selection:{...data.selection.eligible.find((e:any)=>e.nodeId===pending.nodeId),reason:'Test the fixed hypothesis in the eligible slot'}};p.estimatedBudget.attempts=1;}
