...
import { generateWebThreatCertificate, verifyWebThreatCertificate, writeWebThreatCertificate } from "../src/certification/web.js";
...
const artifact = resolve(values.get("artifact") ?? "certification/phase6/web-threat-b9.v1.json");
if (command === "generate") {
  const createdAt = values.get("created-at"); if (!createdAt) throw new Error("--created-at is required");
  const certificate = await generateWebThreatCertificate({ projectRoot, createdAt, signerId: values.get("signer-id") ?? "local_ci" });
  writeWebThreatCertificate(artifact, certificate);
  ...
  const result = await verifyWebThreatCertificate({ projectRoot, artifact });
  ...
  process.stderr.write("Usage: pi-fabric-arbor-web-certify generate|verify [--created-at ISO --artifact PATH --signer-id ID]\n"); process.exitCode = 2;
}
