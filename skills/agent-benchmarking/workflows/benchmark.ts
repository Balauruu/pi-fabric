...
type Stage = "design" | "execute" | "judge" | "adjudicate" | "finalize" | "audit";
...
// Pi Fabric 0.75.0 enforces an effective per-execution ceiling of 100.
...
function parseRequest(raw: string): WorkflowRequest {
  ...
  assert(["design", "execute", "judge", "adjudicate", "finalize", "audit"].includes(value.stage), "stage is invalid");
  const routeStages: { [key: string]: Stage[] } = {
    Design: ["design"], Execute: ["execute"], Audit: ["audit"], Analyze: ["judge", "adjudicate", "finalize"],
  };
  ...
  for (const key of ["design_revision", "execution_revision", "wave_id", "requested_runtime", "requested_model"]) {
    assert(value[key] === null || (typeof value[key] === "string" && value[key].length > 0), `${key} is invalid`);
  }
  for (const key of ["runtime_capability_binding", "call_plan_binding"]) {
...
async function writeOnce(root: string, relative: string, value: unknown, mode: "json" | "jsonl" | "bytes" = "json"): Promise<void> {
...
async function createOrVerifySame(
  root: string,
  relative: string,
  value: unknown,
  ...
  if (await fileState(path) === "missing") {
    await writeOnce(root, relative, value, mode);
    return;
  }
  const suffix = mode === "jsonl" ? ".jsonl" : mode === "json" ? ".json" : ".bin";
...
async function loadStageBindings(root: string, req: WorkflowRequest): Promise<void> {
  if (req.stage === "design" || req.stage === "audit") {
    if (req.runtime_capability_binding !== null) runtimeCapabilities = await loadBoundJson(root, req.runtime_capability_binding, "runtime capability") as unknown as RuntimeCapabilities;
    if (req.call_plan_binding !== null) callPlan = await loadBoundJson(root, req.call_plan_binding, "call plan") as unknown as CallPlan;
    return;
  }
  assert(req.runtime_capability_binding !== null, `${req.stage} requires runtime_capability_binding`);
  const capability = await loadBoundJson(root, req.runtime_capability_binding, "runtime capability") as unknown as RuntimeCapabilities;
  assert(capability && capability.schema_version === 1 && capability.status === "passed" &&
    typeof capability.capability_id === "string" && ID.test(capability.capability_id) &&
    typeof capability.fabric_version === "string" && capability.fabric_version.length > 0 &&
    typeof capability.pi_version === "string" && capability.pi_version.length > 0 &&
    typeof capability.mechanics_version === "string" && capability.mechanics_version.length > 0,
    "runtime capability identity is incomplete");
  unsupported(Number.isInteger(capability.effective_max_calls) && capability.effective_max_calls >= 1 &&
    capability.effective_max_calls <= EFFECTIVE_MAX_AGENT_CALLS, "runtime effective call cap is unsupported");
  assert(Number.isInteger(capability.max_concurrency) && capability.max_concurrency >= 1 &&
    capability.max_concurrency <= HARD_MAX_CONCURRENCY, "runtime concurrency capability is invalid");
  assert(Array.isArray(capability.absolute_log_roots) && capability.absolute_log_roots.length > 0 &&
    capability.absolute_log_roots.every((path) => typeof path === "string" && path.startsWith("/") && !path.includes("..")),
    "runtime absolute log roots are invalid");
  assert(typeof capability.telemetry_projection_version === "string" && capability.telemetry_projection_version.length > 0,
    "runtime telemetry projection version is missing");
  runtimeCapabilities = capability;
  if (req.stage === "finalize") {
    assert(req.call_plan_binding === null, "finalize is zero-call and must not bind a call plan");
    return;
  }
  assert(req.call_plan_binding !== null, `${req.stage} requires call_plan_binding`);
  const plan = await loadBoundJson(root, req.call_plan_binding, "call plan") as unknown as CallPlan;
  ...
  assert(Number.isInteger(plan.max_calls) && plan.max_calls >= 0 && plan.max_calls <= capability.effective_max_calls &&
  ...
  assert(Number.isInteger(plan.max_concurrency) && plan.max_concurrency >= 1 &&
    plan.max_concurrency <= capability.max_concurrency && plan.max_concurrency <= req.max_concurrency,
    "call plan concurrency exceeds a bound capability");
  assert(plan.call_ids.length <= plan.max_calls, "call plan contains more IDs than its call ceiling");
...
async function publishSealReceipt(root: string, binding: SealBinding): Promise<string> {
  ...
  if (await fileState(join(root, relative)) === "missing") {
    await writeOnce(root, relative, value);
  } else {
...
async function archiveAgentLog(root: string, relative: string, result: JsonObject): Promise<JsonObject> {
  assert(typeof result.id === "string" && result.id.length > 0, "Fabric result lacks an agent ID for log capture");
  assert(typeof result.logFile === "string" && result.logFile.startsWith("/"),
    `agent ${result.id} lacks an authoritative absolute logFile path`);
  assert(!result.logFile.includes("//") && !result.logFile.includes("/../") && !result.logFile.includes("/./") &&
    !result.logFile.endsWith("/..") && !result.logFile.endsWith("/."), `agent ${result.id} logFile path is not canonical`);
  assert(result.logFile !== "/home/balauru/.pi/agent" && !result.logFile.startsWith("/home/balauru/.pi/agent/"),
    `agent ${result.id} logFile points into a prohibited profile`);
  const allowedRoots = runtimeCapabilities?.absolute_log_roots ?? ["/tmp/pi-fabric-runs-"];
  assert(allowedRoots.some((prefix) => result!.logFile.startsWith(prefix)),
    `agent ${result.id} logFile is outside runtime-approved absolute roots`);
  await ensureParent(root, relative);
  const target = join(root, relative);
  await rejectSymlinkComponents(result.logFile);
  const copier = "import hashlib,json,os,stat,sys; src,dst=sys.argv[1:3]; " +
    "sfd=os.open(src,os.O_RDONLY|getattr(os,'O_NOFOLLOW',0)); s=os.fstat(sfd); assert stat.S_ISREG(s.st_mode), 'source not regular'; " +
    "fd=os.open(dst,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o600); h=hashlib.sha256(); n=0; " +
    "f=os.fdopen(fd,'wb'); r=os.fdopen(sfd,'rb'); " +
    "exec(\"while True:\\n b=r.read(1048576)\\n if not b: break\\n f.write(b); h.update(b); n+=len(b)\"); " +
    "f.flush(); os.fsync(f.fileno()); f.close(); r.close(); print(json.dumps({'bytes':n,'sha256':h.hexdigest()}))";
  const copied = await requireGate(`python -B -c ${shell(copier)} ${shell(result.logFile)} ${shell(target)}`, `archive Fabric log ${relative}`);
  const metadata = JSON.parse(copied.output) as JsonObject;
  assert(Number.isInteger(metadata.bytes) && metadata.bytes >= 0 && /^[0-9a-f]{64}$/.test(metadata.sha256),
    `archived log metadata is invalid for ${result.id}`);
  return {
    schema_version: 1,
    agent_id: result.id,
    complete: true,
    source: "FabricAgentResult.logFile streamed create-only without guest transport",
    source_path: result.logFile,
...
async function runMeasuredAttempt(root: string, prepared: PreparedAttempt, req: WorkflowRequest): Promise<AttemptOutcome> {
  ...
  try {
    ...
    assignedAt = now();
    await writeOnce(root, `${base}/assignment.json`, {
      ...row,
    ...
    if (assignmentCreated) {
      try {
        await writeOnce(root, `${base}/result.raw.json`, {
          schema_version: 1,
      ...
      if (result && typeof result.id === "string") {
        try {
          log = await archiveAgentLog(root, `${base}/log.raw.jsonl`, result);
          await writeOnce(root, `${base}/log.receipt.json`, log);
          artifacts.push(`${base}/log.raw.jsonl`, `${base}/log.receipt.json`);
        } catch (error) {
          finalizationErrors.push(`log: ${message(error)}`);
        }
      }
      try {
        await writeOnce(root, `${base}/result.json`, resultProjection(result, thrown, log));
      ...
      if (result && Array.isArray(result.nestedAgents) && result.nestedAgents.length > 0) {
        try {
          await writeOnce(root, `${base}/children.json`, {
            schema_version: 1,
            benchmark_id: req.benchmark_id,
            attempt_id: row.attempt_id,
      ...
      if (startedAt !== null && result && typeof result.id === "string") {
        try {
          await writeOnce(root, `${base}/started.json`, {
            schema_version: 1,
            benchmark_id: req.benchmark_id,
            attempt_id: row.attempt_id,
            agent_id: result.id,
      ...
      const mechanismSource = join(root, `workspaces/${row.attempt_id}/${mechanismRelative}`);
      const mechanismState = await fileState(mechanismSource);
      const mechanismRequired = mechanismContract?.exposure !== "not-applicable";
      if (mechanismRequired && mechanismState === "missing") finalizationErrors.push("mechanism: required evidence is missing");
      const mechanismReceipt = {
        schema_version: 1,
        benchmark_id: req.benchmark_id,
        attempt_id: row.attempt_id,
        status: mechanismState === "file" ? "observed" : mechanismRequired ? "missing" : "not-applicable",
        status_qualifiers: mechanismState === "file" ? [] : [mechanismRequired ? "mechanism-unverified" : "not-applicable"],
        predicate: mechanismContract?.predicate ?? null,
        exposure: mechanismContract?.exposure ?? null,
        source_path: mechanismState === "file" ? `workspaces/${row.attempt_id}/${mechanismRelative}` : null,
        source_sha256: mechanismState === "file" ? await sha256File(mechanismSource, `mechanism evidence ${row.attempt_id}`) : null,
      };
      await writeOnce(root, `${base}/mechanism.json`, mechanismReceipt);
      artifacts.push(`${base}/mechanism.json`);

      const status = statusProjection(result, thrown, finalizationErrors);
      const statusQualifiers = [
        ...(mechanismReceipt.status === "missing" ? ["mechanism-unverified"] : []),
      ...
      const terminal = {
        ...
        stage: req.stage,
        status,
        status_qualifiers: statusQualifiers,
      ...
      try {
        await writeOnce(root, `${base}/terminal.json`, terminal);
...
async function reconcile(root: string, schedule: ScheduleRow[], req: WorkflowRequest, canonical: boolean): Promise<JsonObject> {
  const projection = await lifecycleProjection(root, schedule);
  const prefix = canonical ? "" : `checkpoints/${req.request_id}/`;
  const eventsRelative = `${prefix}events.jsonl`;
  const ledgerRelative = `${prefix}ledger.jsonl`;
  await createOrVerifySame(root, eventsRelative, projection.events, "jsonl");
  await createOrVerifySame(root, ledgerRelative, projection.ledger, "jsonl");
...
async function runDesign(root: string, req: WorkflowRequest): Promise<JsonObject> {
  await phase("Frame", { total: 1 });
  await validateSchemas();
  await phase("Inventory", { total: 5 });
  const required = ["protocol.md", "tasks", "conditions", "graders", "schedule.jsonl", "analysis-plan.json"];
  ...
  await phase("Build graders", { total: 1 });
  ...
  return {
    ...
    complete: true,
    failure: null,
    evidence: ["schedule.jsonl", "preflight/grader-certification.json", `${sealRelative}/manifest.json`],
    blockers: [],
  };
}
...
async function runAudit(root: string, req: WorkflowRequest): Promise<JsonObject> {
  ...
  return {
    ...
    status_qualifiers: blockers.length === 0 ? [] : ["reconciliation-incomplete"],
    route: "Audit",
    stage: req.stage,
    request_id: req.request_id,
    benchmark_id: req.benchmark_id,
...
async function runExecute(root: string, req: WorkflowRequest): Promise<JsonObject> {
  ...
  await phase("Preflight conditions", { total: 3 });
  await validateSchemas();
  ...
  await phase("Verify mechanisms", { total: 1 });
  requirePassedGates(gates);

  await phase("Test scheduler and supervisor", { total: 2 });
  const schedule = await loadSchedule(root, req.benchmark_id);

  await phase("Seal execution", { total: 1 });
  ...
  for (const row of selected) {
    ...
    if (terminal === "file") {
      assert(assignment === "file", `terminal without assignment for ${row.attempt_id}`);
      const existingAssignment = await readJson(join(root, `attempts/${row.attempt_id}/assignment.json`));
      assert(existingAssignment.attempt_id === row.attempt_id && existingAssignment.benchmark_id === req.benchmark_id &&
        ...
        `attempt assignment seal binding is stale for ${row.attempt_id}`);
      const existingTerminal = await readJson(join(root, `attempts/${row.attempt_id}/terminal.json`));
      if (existingTerminal.stage !== undefined) await validateContract(root, "attempt", `attempts/${row.attempt_id}/terminal.json`);
      assert(existingTerminal.task_id === row.task_id && existingTerminal.condition_id === row.condition_id &&
        existingTerminal.schedule_revision === row.schedule_revision, `attempt terminal schedule binding failed for ${row.attempt_id}`);
      assert(Array.isArray(existingTerminal.artifact_paths) && existingTerminal.artifact_paths.length > 0,
        `attempt terminal artifact list is empty for ${row.attempt_id}`);
      for (const artifact of existingTerminal.artifact_paths) {
        assertSafePath(artifact, `attempt terminal artifact ${row.attempt_id}`);
        assert(await fileState(join(root, artifact)) === "file", `attempt terminal artifact is unresolved: ${artifact}`);
      }
      skipped.push(row.attempt_id);
    } else if (assignment === "file") {
      ambiguous.push(row.attempt_id);
    } else {
      for (const stale of ["result.raw.json", "result.json", "started.json", "log.raw.jsonl", "log.receipt.json"]) {
        assert(await fileState(join(root, `attempts/${row.attempt_id}/${stale}`)) === "missing",
          `unassigned attempt ${row.attempt_id} has stale ${stale}; launch refused`);
      }
      eligible.push(row);
    }
  }
  assert(ambiguous.length === 0, `assigned without terminal; replay refused: ${ambiguous.join(", ")}`);
  ...
  for (const attempt of prepared) {
    await createOrVerifySame(root, `attempts/${attempt.row.attempt_id}/request.json`, attempt.request, "json");
  }

  await phase("Execute waves", { id: `wave-${wave}`, total: selected.length });
  ...
  if (allTerminal) {
    ...
    await phase("Freeze raw outputs", { total: 1 });
  ...
  const checkpoint = {
    schema_version: 1,
    status: allTerminal && canonicalReconciliation?.complete === true && rawSeal?.status === "passed" ? "complete" :
      blockers.length > 0 ? "blocked" : "checkpoint",
    status_qualifiers: allTerminal ? [] : deferred.length > 0 ? ["call-cap-bounded", "wave-incomplete"] : ["wave-incomplete"],
    stage: req.stage,
    request_id: req.request_id,
    benchmark_id: req.benchmark_id,
  ...
  await writeOnce(root, `checkpoints/${req.request_id}/reconciliation.json`, checkpointReconciliation);
  await writeOnce(root, `checkpoints/${req.request_id}/receipt.json`, checkpoint);
  await phase("Report", { total: 1 });
  return {
    ...checkpoint,
    status: checkpoint.complete ? "complete" : blockers.length > 0 ? "blocked" : "checkpoint",
    status_qualifiers: checkpoint.complete ? [] : blockers.length > 0 ? ["ambiguous-assignment"] : ["bounded-wave-incomplete"],
    route: "Execute",
    stage: req.stage,
...
async function createOrVerifyBlindMaps(root: string, seed: number): Promise<void> {
  ...
  await createOrVerifySame(root, "blind-map.private.json", privateBytes, "bytes");
  await createOrVerifySame(root, "blind-map.public.json", publicBytes, "bytes");
}
...
async function loadExistingGrade(
  ...
  if (grader.kind === "model") {
    ...
    assert(assignment.benchmark_id === record.benchmark_id && assignment.blind_id === record.blind_id &&
      ...
      assignment.design_revision === design.revision && assignment.design_manifest_sha256 === design.manifest_sha256 &&
      assignment.execution_revision === execution.revision && assignment.execution_manifest_sha256 === execution.manifest_sha256 &&
      assignment.request_path === `${base}/request.json` && typeof assignment.assigned_at === "string" &&
      Number.isFinite(Date.parse(assignment.assigned_at)), `model grader assignment mismatch at ${base}`);
  }
  ...
  assert(terminal.benchmark_id === record.benchmark_id && terminal.blind_id === record.blind_id &&
    ...
    terminal.status === record.status && terminal.result_path === `${base}/result.json` &&
...
async function prepareModelGrader(
  ...
  return { blind, grader, item, request: requestValue, base, design, execution };
}
...
async function runModelGrader(root: string, req: WorkflowRequest, job: GradingJob): Promise<JsonObject> {
  ...
  await writeOnce(root, `${job.base}/assignment.json`, {
    schema_version: 1,
    benchmark_id: req.benchmark_id,
    blind_id: blindId,
    grader_id: graderId,
  ...
  try {
    ...
    if (typeof result.id === "string") log = await archiveAgentLog(root, `${job.base}/log.raw.jsonl`, result);
  } catch (error) {
    thrown = message(error);
  } finally {
    await writeOnce(root, `${job.base}/result.raw.json`, {
      schema_version: 1,
      benchmark_id: req.benchmark_id,
      blind_id: blindId,
      grader_id: graderId,
    ...
    if (log !== null) await writeOnce(root, `${job.base}/log.receipt.json`, log);
  }
  ...
  const record: JsonObject = {
    ...
    stage: req.stage === "adjudicate" ? "adjudicate" : "judge",
    status: validOutcome ? value.status : "grader-error",
    status_qualifiers: validOutcome ? ["model-terminal"] : [thrown ? "runtime-error" : "schema-invalid"],
    ...
    evidence_paths: [`blinded/${blindId}/item.json`, `${job.base}/result.raw.json`, ...(log ? [`${job.base}/log.raw.jsonl`, `${job.base}/log.receipt.json`] : [])],
  ...
  await writeOnce(root, `${job.base}/result.json`, record);
  await writeOnce(root, `${job.base}/terminal.json`, {
    schema_version: 1,
    benchmark_id: req.benchmark_id,
    blind_id: blindId,
    grader_id: graderId,
...
async function runAnalyze(root: string, req: WorkflowRequest): Promise<JsonObject> {
  ...
  assert(req.design_revision !== null && req.execution_revision !== null, "Analyze requires design_revision and execution_revision");
  await phase("Freeze raw outputs", { total: 3 });
  await validateSchemas();
  const designBinding = await sealBinding(root, req.design_revision, "design");
  const executionBinding = await sealBinding(root, req.execution_revision, "execution");
  ...
  const schedule = await loadSchedule(root, req.benchmark_id);
  await phase("Reconcile", { total: 1 });
  const executionReconciliation = await reconcile(root, schedule, req, true);
  ...
  await phase("Blind grade", { total: schedule.length });
  ...
  for (const blind of rows) {
    ...
    await createOrVerifySame(root, `blinded/${blind.blind_id}/item.json`, item, "json");
    for (const graderId of task.grader_ids as string[]) {
      ...
      assert(grader.benchmark_id === req.benchmark_id && grader.grader_id === graderId, `grader identity mismatch: ${graderId}`);
      const base = `grader-runs/${blind.blind_id}/${graderId}-${grader.revision}`;
      const key = `${blind.blind_id}/${graderId}@${grader.revision}`;
      gradeOrder.push(key);
      ...
      if (terminalState === "file") {
        ...
        existingGrades.set(key, await loadExistingGrade(root, blind, grader, base, designBinding, executionBinding));
        if (grader.kind === "model") {
          const prepared = await prepareModelGrader(root, req, blind, grader, item, designBinding, executionBinding);
          assert(await fileState(join(root, `${base}/request.json`)) === "file", `resumed model grader request is missing: ${key}`);
          await createOrVerifySame(root, `${base}/request.json`, prepared.request, "json");
        }
      } else {
        ...
        jobs.push(await prepareModelGrader(root, req, blind, grader, item, designBinding, executionBinding));
      }
    }
  }
  const gradesStateBeforeCalls = await fileState(join(root, "grades.jsonl"));
  if (gradesStateBeforeCalls === "file") {
    assert(req.stage === "finalize", "canonical grades.jsonl may only be consumed by the finalize stage");
    assert(jobs.length === 0, "grades.jsonl exists while grader jobs are missing; new grader traffic is forbidden");
    const settled = gradeOrder.map((key) => {
      const grade = existingGrades.get(key);
      assert(grade, `existing grades.jsonl has no settled source for ${key}`);
      return grade;
    });
    await createOrVerifySame(root, "grades.jsonl", settled, "jsonl");
  }
  for (const downstream of ["telemetry.jsonl", "telemetry-aggregate.json", "analysis-input.json", "analysis.json"]) {
    if (jobs.length > 0) assert(await fileState(join(root, downstream)) === "missing", `${downstream} exists before grader completion`);
  }
  ...
  const launchJobs = req.stage === "finalize" ? [] : plannedJobs.slice(0, invocationCapacity);
  const deferredJobs = jobs.filter((job) => !launchJobs.includes(job));
  for (const job of launchJobs) await createOrVerifySame(root, `${job.base}/request.json`, job.request, "json");
  const newGrades = await boundedMap(launchJobs, req.max_concurrency, (job) => runModelGrader(root, req, job));
  for (let index = 0; index < launchJobs.length; index += 1) {
    const job = launchJobs[index];
    existingGrades.set(`${job.blind.blind_id}/${job.grader.grader_id}@${job.grader.revision}`, newGrades[index]);
  }
  if (deferredJobs.length > 0) {
    const checkpointPath = `checkpoints/${req.request_id}/receipt.json`;
    const checkpoint = {
      schema_version: 1,
      status: "checkpoint",
      status_qualifiers: ["call-cap-bounded", "grading-incomplete"],
      route: "Analyze",
      stage: req.stage,
      request_id: req.request_id,
      benchmark_id: req.benchmark_id,
      completed_call_ids: launchJobs.map(jobCallId),
      deferred_call_ids: deferredJobs.map(jobCallId),
      canonical_outputs_published: false,
      complete: false,
      failure: null,
      next_stage: req.stage === "judge" ? "adjudicate" : req.stage,
      blockers: [],
      evidence: launchJobs.map((job) => `${job.base}/terminal.json`),
    };
    await writeOnce(root, checkpointPath, checkpoint);
    return checkpoint;
  }
  ...
  const gradeRecords = gradeOrder.map((key) => {
    const grade = existingGrades.get(key);
    assert(grade, `grade stage did not settle ${key}`);
    return grade;
  });
  assert(gradeRecords.length === gradeOrder.length, "grader matrix did not reconcile exactly");
  ...
  const stagedGradesPath = "analysis/staging/grading-matrix.jsonl";
  await createOrVerifySame(root, stagedGradesPath, gradeRecords, "jsonl");
  if (req.stage !== "finalize") {
    const checkpointPath = `checkpoints/${req.request_id}/receipt.json`;
    const checkpoint = {
      schema_version: 1,
      status: "checkpoint",
      status_qualifiers: [req.stage === "judge" ? "judges-reconciled" : "adjudications-reconciled"],
      route: "Analyze",
      stage: req.stage,
      request_id: req.request_id,
      benchmark_id: req.benchmark_id,
      grade_count: gradeRecords.length,
      required_grade_count: gradeOrder.length,
      canonical_outputs_published: false,
      complete: false,
      failure: null,
      next_stage: req.stage === "judge" ? "adjudicate" : "finalize",
      blockers: [],
      evidence: [stagedGradesPath],
    };
    await writeOnce(root, checkpointPath, checkpoint);
    return checkpoint;
  }
  assert(jobs.length === 0, "finalize is deterministic and cannot launch missing graders or adjudicators");
  await createOrVerifySame(root, "grades.jsonl", gradeRecords, "jsonl");

  await phase("Aggregate telemetry", { total: 1 });
  ...
  await createOrVerifySame(root, "telemetry.jsonl", telemetryRows, "jsonl");
  ...
  if (telemetryRows.length > 0) {
    await requireGate(
      ...
      `--input ${shell(join(root, "telemetry.jsonl"))} --output ${shell(telemetryAggregateScratch)}`,
  ...
  await createOrVerifySame(root, "telemetry-aggregate.json", telemetryAggregate, "json");

  await phase("Analyze", { total: 1 });
  ...
  for (const key of ["control", "candidate", "direction", "practical_threshold", "noninferiority_margin", "seed",
    "bootstrap_draws", "confidence_level", "alternative", "alpha", "multiplicity", "sample_scope", "quality_veto", "integrity_veto"]) {
    if (analysisConfig[key] !== undefined) analysisInput[key] = analysisConfig[key];
  }
  await createOrVerifySame(root, "analysis-input.json", analysisInput, "json");
  assert(scratchPath !== null, "scratch directory is unavailable");
  const analysisScratch = `${scratchPath}/analysis.json`;
  await requireGate(
    `python -B ${shell(`${SKILL_ROOT}/scripts/analyze_paired.py`)} --input ${shell(join(root, "analysis-input.json"))} ` +
  ...
  await createOrVerifySame(root, "analysis.json", analysisResult, "json");
  ...
  const telemetryCoveragePath = `analysis/${req.request_id}/telemetry-coverage.json`;
  await createOrVerifySame(root, telemetryCoveragePath, {
    schema_version: 1,
    benchmark_id: req.benchmark_id,
  ...
  const reconciliationTelemetry = telemetryUnknownAttemptIds.length > 0 ? emptyTelemetry : join(root, "telemetry.jsonl");
  const finalGate = await bash(
    `python -B ${shell(`${SKILL_ROOT}/scripts/reconcile_lifecycle.py`)} --root ${shell(root)} --require-graders ` +
    `--telemetry ${shell(reconciliationTelemetry)} ` +
  ...
  await createOrVerifySame(root, `analysis/${req.request_id}/reconciliation.json`, finalReconciliation, "json");
  ...
  const decisionReport = {
    ...
    reconciliation_path: `analysis/${req.request_id}/reconciliation.json`,
    evidence_paths: ["blind-map.public.json", "grades.jsonl", "telemetry.jsonl", "telemetry-aggregate.json", telemetryCoveragePath, "analysis.json"],
  ...
  const decisionPath = `analysis/${req.request_id}/decision-report.json`;
  await createOrVerifySame(root, decisionPath, decisionReport, "json");
  const persistedDecision = await readJson(join(root, decisionPath));
  assert(persistedDecision.decision === analysisResult.decision && persistedDecision.reconciliation_path === `analysis/${req.request_id}/reconciliation.json`,
    "persisted decision report is incomplete");
  return {
    schema_version: 1,
    status: "complete",
    status_qualifiers: telemetryUnknownAttemptIds.length > 0 ? ["telemetry-partial"] : [],
    route: "Analyze",
    stage: req.stage,
    request_id: req.request_id,
    benchmark_id: req.benchmark_id,
    ...
    complete: true,
    failure: null,
    evidence: ["blind-map.public.json", "grades.jsonl", "telemetry.jsonl", "telemetry-aggregate.json", telemetryCoveragePath, "analysis-input.json", "analysis.json", `analysis/${req.request_id}/reconciliation.json`, decisionPath],
    blockers: [],
  };
}

try {
  request = parseRequest(π.request);
  ...
  scratchPath = `/tmp/pi-agent-benchmark-${request.request_id}`;
  await requireGate(`umask 077; mkdir -- ${shell(scratchPath)}`, "create exclusive request scratch directory");
  const requestPath = await stage("workflow-request.json", π.request);
  await requireGate(
  ...
  receipt = {
    ...receipt,
    schema_version: 1,
    status: unsupportedFailure ? "unsupported" : "failed",
    ...
    status_qualifiers: [unsupportedFailure ? "runtime-unsupported" : "stage-failed"],
    failure: {
      stage: request?.stage ?? receipt.stage,
  ...
  if (request !== null && scratchPath !== null) {
    try {
      ...
      await writeOnce(request.packet_path, failurePath, receipt);
  ...
  if (scratchPath !== null) {
    try {
      ...
      receipt = {
        ...receipt,
        status: "failed",
        status_qualifiers: [...(Array.isArray(receipt.status_qualifiers) ? receipt.status_qualifiers : []), "cleanup-failed"],
        complete: false,
