...
PLANNING_TYPES = {"investigation", "experiment", "decision", "design", "enabler"}
...
def validate(map_dir: Path) -> list[str]:
    ...
    if not map_path.is_file():
        return [f"{map_path}: missing map file"]

    try:
        map_data, map_text = parse_frontmatter(map_path)
    except (OSError, ValidationError) as exc:
        return [f"{map_path}: {exc}"]
    ...
    if residual_without_comments not in {"None.", "- None."}:
        ...
        if not lines or any(
            not re.match(r"^[-*+]\s+", line) or "owner:" not in line.lower() or "destination:" not in line.lower()
            for line in lines
        ):
            errors.append(
                f"{map_path}: every residual-risk bullet must name `owner:` and `destination:`"
            )
    ...
    for path in ticket_paths:
        ...
        unknown = sorted(set(data) - TICKET_FIELDS)
        if unknown:
        ...
        if status == "closed":
            ...
            if UNCHECKED_RE.search(section(text, "Acceptance")):
        ...
        needs_revision = (
            ...
            status in {"active", "review"} and ticket_type in {"verification", "release"}
        ) or (
            status == "closed" and disposition == "completed" and ticket_type in {"verification", "release"}
        )
        if needs_revision:
        ...
        if ticket_type == "enabler" and result == "ready" and expiry_time is not None and expiry_time <= now:
            errors.append(f"{path}: ready enabler expired at `{expiry}`; record result `expired`")
    ...
    for ticket_id, (data, path) in tickets.items():
        ...
        status = str(data.get("status", ""))
        disposition = str(data.get("disposition", ""))
        must_be_ready = status in {"active", "review"} or (
        ...
        release_needs_pass = data.get("type") == "release" and (
            status in {"active", "review"} or (status == "closed" and disposition == "completed")
        )
...
''',
        ...
        negative_errors = validate(root)
        if not any("unsatisfied requirement `VER-01:pass`" in error for error in negative_errors):
...
def main() -> int:
    ...
    errors = validate(args.map_dir.resolve())
    if errors:
        for error in errors:
