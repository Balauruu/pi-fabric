...
└── support/acceptance/      # only when final acceptance is sharded
    └── <shard>.md
```
...
| `support/acceptance/<shard>.md` | Assigned final-acceptance shard only: bounded source/coverage/contract checks. Reserve each safe path before launch. It returns control findings to workflow code and never writes `RESEARCH.md`. No-write mode keeps the same outputs in memory. |
...
1. **Plan only an actual unknown decomposition:** for an explicit question whose requirements already partition the uncertainties, create and validate the assignment index in code and launch no planner. Otherwise one planner returns only a bounded control plan, never report prose. Workflow code owns the brief, canonical self-contained Requirement contract, reservations and plan in `state.json`; validate every selected record against the canonical fields before fan-out. A substantive discovery note, if any, is returned for workflow persistence at a reserved stream path.
2. **Research independently:** reserve named, safe `streams/<assignment>.md` paths and the exact downstream launch allowance in serialized control state before launch. Launch only non-overlapping source investigations from `researcher.md` in bounded parallel batches. Each terminal result is status-checked, saved verbatim by workflow code, and equality-read back; preserve nonempty partials and successful siblings. Empty evidence is a gap. Stop new fan-out on systemic failure rather than repeating a batch. Do not send counterpart reports or their findings to fresh workers.
3. **Reconcile and synthesize once:** after all available stream notes are persisted, one reconciliation/composer worker reads every full note and relevant support continuation, independently checks original decisive sources while composing, applies the evidence gates, and alone writes the clean task-specific `RESEARCH.md`. This is the first synthesis gate, not a mandatory pre-synthesis verifier or disposition bottleneck. It returns compact requirement dispositions and ranked, decision-changing gaps to workflow code. It retains source-unique methods, results, conditions, counterevidence, unknowns and operational artifacts, binds each material claim to its source, and includes the complete retained-source appendix. It never puts brief/control/phase/validation prose into the report.
4. **Independent final acceptance:** a source/coverage/contract validator other than the composer reads `RESEARCH.md`, the full source notes and decisive original passages. It returns only a compact acceptance or bounded correction/gap request to workflow code. It checks all question dimensions, source entailment, quantitative comparability, counterevidence, unknowns, source appendix, links, ownership and false-completion gates. A report existing or a composer success is not acceptance.
5. **Targeted repair and recheck:** only a decision-changing validator gap can launch bounded gap research or a correction. Preserve the original report as a pre-correction copy, preserve all sibling notes, reserve a distinct repair stream path, and ask the same sole report-author role to replace `RESEARCH.md` without administrative filler. Then a different final validator rechecks the changed evidence and report. Stop on sufficient support, no material progress, exhausted allowance or blocked access. If no source was inspected, return blocked rather than prose success; remaining gaps stay qualified/unknown/blocked.

The topology is parallel independent research -> one source-checking reconciliation/composer -> independent final acceptance -> targeted repair/recheck. Planning is conditional, never ritual. The task ledger records exact stream count, planned launch ceiling, actual launches, model/thinking/status and phase elapsed time. Every required question ends with checked support, a qualified answer, an explicit gap or a blocked conclusion.
