...
The assignment states persisted or no-write mode. In no-write mode, all file-read/write instructions below refer instead to the full evidence and role outputs passed inside the program: the reconciliation/composer consumes substantive research, checks decisive sources and returns the report text plus compact dispositions; the independent validator consumes the report and full evidence and returns acceptance or correction requests. No role writes files or returns saved paths. Keep each handoff within its task budget without replacing evidence with metadata; if full required support cannot fit, return partial/blocked coverage. The final validated report may return inline to Main for relay. Persisted mode uses the dossier and document ownership below.

## Reconcile, synthesize, then independently accept

There is no mandatory separate verifier/disposition handoff before synthesis. After independent research is complete, the single reconciliation/composer reads every full saved stream, including useful partials, follows support links and continuations, and independently inspects original decisive sources while composing. Compact receipts are navigation only, not evidence. The composer applies the source, quantitative-comparability, causal, transfer and contradiction gates to each material report claim. It preserves source-unique methods, results, conditions, counterevidence, unknowns and operational artifacts, rather than retaining a renamed checklist or a stream summary.

The composer alone writes the one clean `RESEARCH.md` from a task-specific outline. It returns compact requirement dispositions and ranked decision-changing gaps to workflow code, but writes no administrative control paragraphs, validation claims, phase narration or pending-revalidation prose into the report. For decision-grade work it includes the complete retained-source appendix. A source name, worker success or polished prose does not establish support.

An independent final source/coverage/contract validator then reads `RESEARCH.md`, all full source notes and decisive original passages. It checks question coverage, source entailment, source/claim binding, retained method/result/condition detail, comparability, counterevidence, unknowns, links, appendix completeness, ownership and false-completion gates. It returns only acceptance, qualified coverage and bounded correction or gap requests to workflow code. It never edits the report or calls its own work independent.

Only a decision-changing final-validator gap permits repair. Workflow code reserves a distinct repair note path and preserves successful siblings and the pre-correction `RESEARCH.md`; a gap researcher investigates only that gap, then the sole composer replaces the report. A different final validator rechecks affected evidence and the replacement. No repair is launched for cosmetic wording, an unranked source list or a generic desire for more research. Stop on sufficient support, no material progress, exhausted allowance or blocked access. If no source was inspected, the result is blocked, not prose success. Every requirement remains supported, qualified, unknown or blocked.

## Synthesize from the research documents

The composer reads substantive findings, source notes, methods/results, counterevidence and gaps in every assigned stream before drafting. Follow supporting-file links and read omitted sections in bounded ranges. Researchers return full Markdown notes, which workflow code persists unchanged. Do not ask Main to absorb the corpus.
...
The report validator must be independent of the report author. Read back RESEARCH.md and inspect decisive linked evidence and composer dispositions. Check:
