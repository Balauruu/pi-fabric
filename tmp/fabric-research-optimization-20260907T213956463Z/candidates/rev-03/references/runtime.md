...
if (persist && hasText) {
  try {
    ...
    // pi.read is bounded and can truncate large text. Read line-bounded chunks and reject
    // an uncheckable oversized line rather than claiming an incomplete read equals the note.
    const lineCount = Number((await pi.bash({ cmd: `wc -l < "${streamPath}"` })).output.trim());
    let readBack = "";
    for (let offset = 1; offset <= lineCount + 1; offset += 800) {
      ...
      if (!page && (response.includes("Use offset=") || response.includes("exceeds 50.0KB limit"))) {
        throw new Error("Research read-back exceeded bounded line retrieval");
      }
      // The continuation marker follows the line terminator it hides; restore that terminator.
      readBack += page ? page[1] + "\n" : response;
    }
