...
Honor user-specified streams, counts and output structure. Otherwise choose only as many streams as there are useful independent questions. Main does the framing; there is no separate planning agent. A normal substantive run uses one worker per stream and one synthesizer.
...
When planned streams finish or fail, proceed with the available evidence. Missing evidence remains a stated limitation. Do not relaunch streams, add follow-up investigations or cycle through report rewrites.
