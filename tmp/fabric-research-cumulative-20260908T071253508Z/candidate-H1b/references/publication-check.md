...
Return one verdict: `accept`, `correct`, or `reject`. For each material defect return an ID, exact report passage, decisive source passage and locator, why the defect changes interpretation or action, and the smallest supported correction. For every proposed factual replacement, inspect and quote the original report's own cited source location as well as the passage supporting the proposed replacement. Do not change a value while retaining a citation that supports a different value. Compare the abstract/main text with the cited method, table or appendix when they describe the same measurement. If either location is unavailable, narrow the claim or mark the correction unresolved. If primary passages conflict, the correction must disclose the conflict and keep each value with its own locator, not replace one with a supposedly settled number. The correcting synthesizer and rechecker must verify both sides of this same citation-local comparison before accepting the replacement. Missing evidence is an explicit gap, not a requested invented answer. Do not request stylistic rewrites, source quotas or retention of every source. Use `reject` when the available evidence cannot support a bounded correction.
