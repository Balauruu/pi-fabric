...
const outcomes = await parallel<Outcome>(batch.map(a => async (): Promise<Outcome> => {
  ...
  try {
    ...
    await pi.write({path, text: JSON.stringify(r)});
    return {assignment: a.id, path, id: r.id, status: r.status, error: r.error ?? null,
      usage: r.usage, hasValue: r.value !== undefined};
  } catch (error) {
    const outcome = {assignment: a.id, path, id: null, status: "indeterminate", error: String(error)};
    await pi.write({path, text: JSON.stringify(outcome)});
    return outcome;
  }
