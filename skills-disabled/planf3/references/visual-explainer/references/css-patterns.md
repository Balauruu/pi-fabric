...
Use real `<table>` elements only for two-dimensional relationships. Tables are fit-first:

1. Split unrelated comparisons and move long explanations into adjacent prose.
2. Let prose cells wrap and use the page's wide content column.
3. Use deliberate column widths when the content shape is known.
4. Only an irreducible comparison table gets a horizontal scroller.

Page tables never get a fixed height, `max-height`, nested vertical scrolling, hidden columns, or ellipsized meaningful values. Keep `<caption>`, `<th>`, and `scope` associations. When horizontal scrolling is necessary, label and focus the wrapper so keyboard users can reach it:

```html
<div class="table-wrap">
  <div class="table-scroll" role="region" aria-labelledby="decision-caption" tabindex="0">
    <table class="data-table">
      <caption id="decision-caption">Decision comparison</caption>
      <!-- headers and rows -->
    </table>
  </div>
</div>
...
Do not add `.table-scroll` around a reasonable table that fits after wrapping.
